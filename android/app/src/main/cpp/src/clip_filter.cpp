...
namespace kp {
    namespace {
        ...
        static bool contains_ci(const std::string& s, const char* sub) {
            ...
            for (size_t i = 0; i + n <= s.size(); ++i) {
                ...
                for (size_t j = 0; j < n; ++j) {
                    if (ascii_lower(s[i + j]) != sub[j]) { ok = false; break; }
                }
                if (ok) return true;
            }
            return false;
        ...
        static bool looks_like_email(const std::string& s) {
            ...
            if (dot == std::string::npos || dot + 2 >= s.size()) return false;
            return true;
        }
    ...
    ClipSkip classify(const std::string& raw) {
        const std::string s = kt::normalize(raw);

        // 2. Пустой или слишком короткий
        ...
        for (const char* kw : keywords) {
            if (contains_ci(s, kw)) { ++code_kw; if (code_kw >= 2) break; }
        }
        ...
        const double code_ratio = (double)(st.code_chars + code_kw * 5) / (double)st.total_cp;
        if (code_ratio > 0.25 || code_kw >= 2) {
            return ClipSkip::CODE;
        }

        const double text_ratio = (double)st.letters / (double)st.total_cp;
        if (text_ratio < 0.40) {
