...
// ── Декодирование UTF-8 кода ─────────────────────────────
    static uint32_t next_cp(const std::string& t, size_t& i) {
    ...
    std::string detect_script(const std::string& t) {
        ...
        if (tk > 0) return "tk";
        int mx = 0; std::string best = "lat";
        auto consider = [&](int v, const char* n) { if (v > mx) { mx = v; best = n; } };
        consider(cyr, "cyr"); consider(lat, "lat"); consider(ara, "ara");
        consider(cjk, "cjk"); consider(dev, "dev");
    ...
    std::string normalize(const std::string& t) {
        ...
        for (char ch : t) {
            if (ch == '\n' || ch == '\r') {
                ...
                if (started && !sp && !nl) { out += ' '; sp = true; }
            } else {
                started = true; sp = false; nl = false;
                out += ch;
            }
        }
    ...
    static bool is_sep_at(const std::string& s, size_t pos) {
    ...
    std::string split_chunks(const std::string& text, int32_t max) {
        ...
        while (start < text.size()) {
            size_t end = start + (size_t)max;
            if (end >= text.size()) { end = text.size(); }
            else {
                size_t cut = std::string::npos;
                for (size_t p = end; p > start + max / 2; --p) {
                    if (is_sep_at(text, p) && p + 1 < text.size() && text[p + 1] == ' ') { cut = p + 2; break; }
                    if (text[p] == '\n') { cut = p + 1; break; }
                }
                if (cut == std::string::npos) {
                    for (size_t p = end; p > start + max / 2; --p)
                        if (text[p] == ' ') { cut = p + 1; break; }
                }
    ...
    static std::string enc4(uint32_t cp) {
        std::string s;
        s += (char)(0xF0 | (cp >> 18));
    ...
    std::string flag_emoji(const std::string& cc) {
        if (cc.size() != 2) return "\xF0\x9F\x8F\xB3\xEF\xB8\x8F"; // 🏳️
        const uint32_t base = 0x1F1E6 - 'A';
        return enc4(base + (uint32_t)toupper((unsigned char)cc[0])) +
               enc4(base + (uint32_t)toupper((unsigned char)cc[1]));
    }
}