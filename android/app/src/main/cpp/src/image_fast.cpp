...
namespace kp {
    ...
    int32_t resize_image(const char* src, const char* dst, int32_t max_side, int32_t quality) {
        ...
        if (nw != w || nh != h) {
            ...
            std::vector<int> xs((size_t)nw);
            std::vector<int> ys((size_t)nh);
            for (int x = 0; x < nw; ++x) xs[(size_t)x] = (int)((double)x * w / nw);
            for (int y = 0; y < nh; ++y) ys[(size_t)y] = (int)((double)y * h / nh);

            for (int y = 0; y < nh; ++y) {
                const unsigned char* srow = px + (size_t)ys[(size_t)y] * w * 4;
                unsigned char* drow = resized + (size_t)y * nw * 4;
                for (int x = 0; x < nw; ++x) {
                    std::memcpy(drow + (size_t)x * 4, srow + (size_t)xs[(size_t)x] * 4, 4);
                }
            }
            out = resized;
        }
...
namespace {
    ...
    bool ocr_load_gray(const char* path, int orient, OcrImg& out) {
...
#if defined(KP_HAS_ASM_KERNELS)
        if (orient == 1 && W == w && H == h && total_pixels > 0 && total_pixels <= HARD_PIXEL_LIMIT) {
            ...
            LOGD("ocr_load_gray: dst_aligned=%d src_aligned=%d pixels=%lld",
                 dst_aligned, src_aligned, (long long)total_pixels);

            if (dst_aligned && src_aligned) {
                LOGD("ocr_load_gray: using ASM kernel for %lld pixels", (long long)total_pixels);
        ...
        for (int y = 0; y < H; ++y) {
            unsigned char* grow = g.data() + (size_t)y * W;
            for (int x = 0; x < W; ++x) {
                int sx = x, sy = y;
                switch (orient) {
                ...
                grow[x] = (unsigned char)std::min(255.f, std::max(0.f, yv));
            }
        }
    ...
    void ocr_resize_gray(OcrImg& im, int nw, int nh) {
        ...
        const unsigned char* src = im.px.data();
        const int sw = im.w, sh = im.h;
        ...
        for (int y = 0; y < nh; ++y) {
            ...
            const float w0y = 1.0f - ty, w1y = ty;
            const unsigned char* r0 = src + (size_t)y0 * sw;
            const unsigned char* r1 = src + (size_t)y1 * sw;
            unsigned char* orow = out.data() + (size_t)y * nw;
    ...
    void ocr_median3(OcrImg& im) {
        ...
        for (int y = 0; y < h; ++y) {
            ...
            unsigned char* orow = out.data() + (size_t)y * w;
            ...
            for (int x = 0; x < w; ++x) {
                ...
                for (int yy = y0; yy <= y1; ++yy) {
                    const unsigned char* r = src + (size_t)yy * w;
                    for (int xx = x0; xx <= x1; ++xx) win[k++] = r[xx];
                }
    ...
    void ocr_rotate_gray(OcrImg& im, int angle) {
        ...
        if (angle == 180) {
            std::reverse(im.px.begin(), im.px.end());
            return;
        }
        ...
        for (int y = 0; y < H; ++y) {
            const unsigned char* srow = src + (size_t)y * W;
            for (int x = 0; x < W; ++x) {
                if (angle == 90) out[(size_t)x * NW + (NW - 1 - y)] = srow[x];
                else out[(size_t)(W - 1 - x) * NW + y] = srow[x];
            }
        }
