...
[[gnu::cold]] static void dump_backtrace() {
    ...
    for (int i = 0; i < frame_count; ++i) {
        ...
        if (dladdr(addr, &info) && info.dli_sname) {
            unsigned long offset = (unsigned long)((char*)addr - (char*)info.dli_saddr);
            __android_log_print(ANDROID_LOG_FATAL, CRASH_TAG,
                                " #%02d pc %p %s + 0x%lx (%s)",
...
[[gnu::cold]] static void crash_handler(int sig, siginfo_t* info, void* context) {
    ...
    __android_log_print(ANDROID_LOG_FATAL, CRASH_TAG, " Address: %p", info ? info->si_addr : nullptr);
    __android_log_print(ANDROID_LOG_FATAL, CRASH_TAG, " PID: %d", (int)getpid());
    __android_log_print(ANDROID_LOG_FATAL, CRASH_TAG, " TID: %d", (int)gettid());
    if (info) {
